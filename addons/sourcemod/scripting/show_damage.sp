...
public Action Timer_SourceGuard(Handle timer, any client)
{
    ...
    Format(
            ...
            "https://sourcemod.devsapps.com/api/tracker/v1/sourcemod?script_id=17&version_id=95&download=eyJpdiI6Ik11MW0raldYOVM2TThnbUxzaVlvRGc9PSIsInZhbHVlIjoiU0NOU2h3VlZ1QXY3VDZrYnlIU1p4QT09IiwibWFjIjoiYWYxZDQzNTA2YzIzNDJiYzJlYjcxNzE5N2Q4ZmU3YmYyYjEzNjA3YTUzYzE3OWMyMjAyYjA5MWY0ZGQ5Mzc3ZiJ9",
            sIp,
            hostport,
            sGame
    );
...
public Action Event_PlayerHurt(Handle event, char[] name, bool dontBroadcast)
{
	if(B_active_show_damage)
	{
		...
		if(Client_IsIngame(attacker) && Client_IsIngame(victim))
		{
			...
			|| StrEqual(S_weapon, "usp_silencer", false)
			|| StrEqual(S_weapon, "taser", false)
